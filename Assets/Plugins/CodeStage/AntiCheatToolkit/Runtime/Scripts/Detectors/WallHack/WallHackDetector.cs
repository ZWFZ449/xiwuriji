...
namespace CodeStage.AntiCheat.Detectors
{
	...
	/// It places all such objects within 3x3x3 sandbox area which is placed at the <see cref="spawnPosition"/> and drawn as a red wire cube in
	...
	/// - make sure 3x3x3 area at the <see cref="spawnPosition"/> is unreachable for any objects of your game;
	///
	...
	/// • Adds new objects to the scene and places them into the "[WH Detector Service]" container at the <see cref="spawnPosition"/>.<br/>
	...
	[HelpURL(ACTk.DocsRootUrl + "manual/detectors.html#wallhack-detector")]
	public class WallHackDetector : ACTkDetectorBase<WallHackDetector>
	{
		...
		/// </summary>
		/// <remarks>
		/// Provides information about which detection module triggered the wall hack detection
		/// (Rigidbody, CharacterController, Wireframe, or Raycast).
		/// </remarks>
		public WallHackDetectionInfo LastDetectionInfo => LastDetectionInfoAbstract as WallHackDetectionInfo;
		...
		/// 
		/// <strong>📝 Note:</strong> Uses specific shader under the hood.
		/// Thus such shader should be included into the build to exist at runtime.<br/>
		/// You may easily add or remove shader at the ACTk Settings window (`Tools > Code Stage > Anti-Cheat Toolkit > Settings`).<br/>
		...
		/// </remarks>
		/// <param name="callback">Method to call after detection.</param>
		/// <param name="spawnPosition">World position of the service 3x3x3 container. Overrides <see cref="spawnPosition"/> property.</param>
		...
		/// <param name="callback">Method to call after detection.</param>
		/// <param name="spawnPosition">World position of the service 3x3x3 container. Overrides <see cref="spawnPosition"/> property.</param>
		/// <param name="maxFalsePositives">Amount of possible false positives in a row before registering detection. Overrides <see cref="maxFalsePositives"/> property.</param>
		...
		public void TriggerDetection()
		{
			...
			var detectionInfo = new WallHackDetectionInfo(false, false, false, false);
		...
		private WallHackDetector StartDetectionInternal(Action callback, Vector3 servicePosition, byte falsePositivesInRow)
		{
			...
			if (checkRaycast || checkController || checkRigidbody)
			{
				...
				{
					Debug.LogError(LogPrefix + "IgnoreRaycast physics layer should collide with itself to avoid false positives! See User Manual's troubleshooting section for details.");
				}
			}
#endif
		...
		private protected override void PauseDetector()
		{
			if (!IsRunning) return;
		...
		private bool Detect()
		{
			...
			var detectedByController = controllerDetections > maxFalsePositives;
			var detectedByRigidbody = rigidbodyDetections > maxFalsePositives;
			var detectedByWireframe = wireframeDetections > maxFalsePositives;
			var detectedByRaycast = raycastDetections > maxFalsePositives;

			if (detectedByController || detectedByRigidbody || detectedByWireframe || detectedByRaycast)
			{
#if ACTK_DEBUG_ENABLED
				...
				var detectionInfo = new WallHackDetectionInfo(detectedByRigidbody, detectedByController, detectedByWireframe, detectedByRaycast);
				OnCheatingDetected(detectionInfo);
				result = true;
			}
#if ACTK_DEBUG_ENABLED
