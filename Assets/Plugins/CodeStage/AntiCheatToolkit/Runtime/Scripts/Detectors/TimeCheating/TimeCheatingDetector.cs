...
namespace CodeStage.AntiCheat.Detectors
{
	...
	/// Automatically switches to the current domain on WebGL (if <see cref="RequestUrl"/> leads to any external resource) to avoid CORS limitation.<br/>
	...
	///
	/// <strong>⚠️ Important:</strong> Avoid using detectors from code at the Awake phase.
	/// 
	/// <example>
	/// <code>
	/// <![CDATA[
	...
	[HelpURL(ACTk.DocsRootUrl + "manual/detectors.html#time-cheating-detector")]
	public class TimeCheatingDetector : ACTkDetectorBase<TimeCheatingDetector>
	{
		...
		public enum CheckResult
		{
			...
			/// <summary>
			/// Direct time cheating was not detected but difference between local and online UTC clocks exceeds the <see cref="wrongTimeThreshold"/>.
			/// </summary>
			WrongTimeDetected = 10,

			/// <summary>
			/// Subsequent measurements difference of local and online time difference exceeded the <see cref="realCheatThreshold"/>.
			/// </summary>
			CheatDetected = 15,

			/// <summary>
			/// There was error while making cheat check, please check <see cref="LastError"/> for details.
			/// </summary>
			Error = 100
		}
		...
		public enum ErrorKind
		{
			...
			/// Please make sure <see cref="IsCheckingForCheat"/> == false before trying to force another cheat check.
			/// </summary>
		...
		/// </remarks>
		public TimeCheatingDetectionInfo LastDetectionInfo => LastDetectionInfoAbstract as TimeCheatingDetectionInfo;
		...
		/// <summary>
		/// Last check result. Check <see cref="LastError"/> if this has value of CheckResult.Error.
		/// </summary>
		...
		#region private and private protected fields
		...
		/// Creates a new detector instance if it doesn't exists in scene.
		/// </remarks>
		/// <param name="intervalMinutes">Time in minutes between checks. Overrides <see cref="interval"/> property.</param>
		/// <param name="cheatCheckedEventHandler">Method to call after each cheat check. Pass null if you wish to use event, set in detector inspector.</param>
		...
		// (C) Dmitry Yuhanov | codestage.net
		...
		/// <summary>
		/// Allows to manually execute cheating check. Restarts <see cref="interval"/>.
		/// </summary>
		/// <remarks>
		...
		/// <summary>
		/// Allows to manually execute cheating check and wait for the completion within coroutine. Restarts <see cref="interval"/>.
		/// </summary>
		/// <remarks>
		/// Use inside of the coroutine and check <see cref="LastResult"/> property after yielding this method.
...
#if ACTK_ASYNC
		/// <summary>
		/// Allows to manually execute cheating check and wait for the completion within async method. Restarts <see cref="interval"/>.
		/// </summary>
		/// <remarks>
		...
		private protected override bool Init(TimeCheatingDetector instance)
		{
			if (cachedUri == null)
				cachedUri = UrlToUri(requestUrl);
		...
		private protected override bool DetectorHasListeners()
		{
		...
		private void ReportCheckResult()
		{
			...
			switch (LastResult)
			{
				...
				case CheckResult.CheatDetected:
					var detectionInfo = new TimeCheatingDetectionInfo(LastResult, LastError);
