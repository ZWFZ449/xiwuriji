...
public class TowerBabel_skill : Base_Mono
{
    private enum TowerBabel_skill_type
    { 
    人物元素=1,
    主动技能,
    被动技能,
    ...
    private void Init()
    {
        ...
        for (int i = 0; i < SumSave.db_towerbabels.Count; i++)
        {
            TowerBabel_skill_item item = Instantiate(TowerBabel_skill_item_prefab, m_btn_brom);
            item.Data = SumSave.db_towerbabels[i];
            TowerBabel_skill_item_dic.Add(SumSave.db_towerbabels[i].TowerBabel_name, item);
    ...
    private void ShowBtn()
    {
        ...
        switch ((TowerBabel_skill_type)crt_skill.Data.TowerBabel_type)
        {
            case TowerBabel_skill_type.人物元素:
            case TowerBabel_skill_type.被动技能:
            case TowerBabel_skill_type.宠物天赋:
                ...
                break;
            case TowerBabel_skill_type.主动技能:
                c = lv > 0 ? HexToColor("#70ff69") : HexToColor("#808080");
                Get().Init(("[" + "激活加成" + "]"), c);
                Show_active_skill(crt_skill.Data.activate_offect, lv, true, c);
                if (lv > 0)
                {
                    c = HexToColor("#00ffff");
                    Get().Init(("[" + "等级加成" + "]"), c);
                    Show_active_skill(crt_skill.Data.up_offect, lv, false, c);
                }
                UpShow(lv, c);
                break;
            default:
    ...
    private void OnClick_btn(btn_item item)
    {
        ...
        if (Return_Condition())
        {
            ...
            SumSave.crt_user_towerbabel.SetSKill(crt_skill.Data);
